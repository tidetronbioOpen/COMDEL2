...
def construct_dataset(data, config):
    ...
    data_loader = MyDataSet(input_ids, origin_input_ids, labels)
...
def load_data_one_pathways(config):
    ...
    token_list_total, origin_token_list_total, max_len_total = transform_token2index(sequences, config)
    config.max_len_train = max_len_total
    config.max_len_test = max_len_total
    config.max_len = max_len_total

    data_total = make_data_with_unified_length(token_list_total, origin_token_list_total, labels, config)
