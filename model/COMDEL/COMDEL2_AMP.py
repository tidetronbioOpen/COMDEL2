...
#     plt.close()  # Close the plot to release memory
    ...
    plt.figure(figsize=(20, 5))
    # 绘制每个位置的预测值，并加粗线条
    plt.plot(positions, predictions, linewidth=10, label='AMP', color='black')
    # 去除顶部和右侧的边框
    ax = plt.gca()
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ...
    plt.ylabel('AMP', fontsize=25, fontweight='bold',color='black')
    plt.title(f'Threshold ({threshold})', fontsize=30, fontweight='bold', loc='left')
    # 添加图例
    #plt.legend(fontsize=20)
    # 调整布局以适应标签
    plt.tight_layout()
    plt.savefig(plt_results+'/'+'AMP_predictions.png', format='png', bbox_inches='tight')#plt.savefig('AMP_predictions.png', format='png', bbox_inches='tight')
    # plt.show()
    plt.close()
    # # 创建绘图
    # plt.figure(figsize=(20, 5))
