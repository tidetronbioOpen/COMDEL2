...
def load_data(config):
    ...
    if if_multi_scaled:
        ...
        if config.path_test_data is not None:
            train_iter_orgin, test_iter = data_loader_kmer.load_data_two_pathways(config)
        else:
            train_iter_orgin, test_iter = data_loader_kmer.load_data_one_pathways(config)
        # config.max_len = 256
    else:
        ...
        config.token2index = residue2idx
        if config.path_test_data is not None:
            train_iter_orgin, test_iter = data_loader.load_data_two_pathways(config)
        else:
            train_iter_orgin, test_iter = data_loader.load_data_one_pathways1(config)
...
def model_eval(data_iter, model, criterion, config):
    ...
    # Plot ROC curve
    # plt.plot(roc_data[0], roc_data[1], label='ROC curve (AUC = %0.3f)' % roc_data[2])
    # plt.xlabel('False Positive Rate')
    # plt.ylabel('True Positive Rate')
    # plt.legend()
    # plt.savefig('roc_curve.pdf', format='pdf', bbox_inches='tight')
    # plt.show()
    # df = pd.DataFrame.from_dict({'x': roc_data[0], 'y': roc_data[1]})
    # df.to_excel('Less150_0.992.xlsx', index=False)
...
def select_dataset():
    ...
    path_test_data = None #'../data/ACP_dataset/tsv/ACP-Mixed-80-test.tsv'
...
def load_config():
    ...
    if model_name == 'Basic' or model_name == 'BiLSTM':
